...
export default async function loadInOutBoardHistoryTotals(user, args) {
  ...
  const userTotals = [];
  for (const statusDoc of statusesRes) {
    const minutes = statusDoc.end ? (statusDoc.end - statusDoc.start) / 60000 : 0;

    let userEntry = userTotals.find(u => u.userId === statusDoc.userId);
    if (!userEntry) {
      const ownerDoc = ownersRes1.find(owner => owner._id === statusDoc.userId);
      userEntry = {
    ...
    let statusEntry = userEntry.statusTotals.find(s => s.statusId === statusDoc.status);
    if (!statusEntry) {
      statusEntry = {
        statusId: statusDoc.status,
        minutesTotal: 0,
        hoursTotal: 0,
      };
      userEntry.statusTotals.push(statusEntry);
    }
    statusEntry.minutesTotal += minutes;
  }
  for (const userEntry of userTotals) {
    for (const statusEntry of userEntry.statusTotals) {
      statusEntry.hoursTotal = statusEntry.minutesTotal / 60;
    }
  }
  // /build per-user totals

  // join options
  const userTotalsWithOptionsJoined = userTotals.map(userEntry => ({
    ...userEntry,
    statusTotals: userEntry.statusTotals.map(statusEntry => {
      const option = inOutOptions.find(opt => opt.id === statusEntry.statusId);
      return {
        ...statusEntry,
        text: option?.text,
        work: option?.work,
  ...
  return {
    userTotals: userTotalsWithOptionsJoined,
    meta,
    limit: limitInfo,
    inOutOptions,
  };
